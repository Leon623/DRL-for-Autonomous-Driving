    ...
    def __init__(self, observation_space: gym.spaces.Dict):
        ...
        for key, subspace in observation_space.spaces.items():
            if key == "camera":
                ...
                extractors[key] = nn.Sequential(nn.MaxPool2d(1), nn.Flatten())
                total_concat_size += subspace.shape[1] // 1 * subspace.shape[2] // 1
...
def main():
  ...
  model = SAC("MultiInputPolicy", env,buffer_size=5000, verbose=1,policy_kwargs=policy_kwargs,tensorboard_log="./td3_cartpole_tensorboard/")
  model.learn(total_timesteps=10000, log_interval=5)
  model.save("td3_pendulum")

  model = SAC.load("td3_pendulum",env=env,verbose=1, tensorboard_log="./td3_cartpole_tensorboard/",policy_kwargs=policy_kwargs)
